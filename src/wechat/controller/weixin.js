...
export default class extends Base {
    ...
    async oauthAction() {
            ...
            if (is_weixin(this.userAgent()) && think.isEmpty(openid)) {
                ...
                var oauthUrl = pingpp.wxPubOauth.createOauthUrlForCode(this.setup.wx_AppID, `http://${this.http.host}/wechat/weixin/getopenid?showwxpaytitle=1`);
    ...
    async getopenidAction() {
        ...
        let wx_user = await this.model("wx_user").where({
            openid: openid
        }).find();
        ...
        if (think.isEmpty(wx_user)) {
            ...
            await this.model("wx_user").where({
                openid: openid
            }).update(userinfo);
            ...
            if (think.isEmpty(wx_user.uid)) {
                ...
                await this.spiderImage(userinfo.headimgurl, filePath + '/avatar.png')
                    //绑定直接登陆
                let last_login_time = await this.model("member").where({
                    id: wx_user.uid
                }).getField("last_login_time", true);
    ...
    async signinAction() {
            ...
            let wx_info = await this.model("wx_user").where({
                openid: open_id
            }).find();
    ...
    async organizingAction() {
            ...
            if (think.isEmpty(data.username)) {
                ...
                res = await this.model("member").where({
            ...
            console.log(data);
            await this.model("member").autoLogin({
                id: reg
            }, this.ip()); //更新用户登录信息，自动登陆
            let wx_userInfo = {
                'uid': reg,
